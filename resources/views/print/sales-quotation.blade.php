...
<head>
    <meta charset="utf-8">
    <title>Quotation - {{ $sale->invoice_number }}</title>
    <style>
        ...
        .alt-row:nth-child(even) { background: #f9fafb; }
    </style>
</head>
<body>
    @php
        ...
        <table class="header-table">
            <tr>
                ...
                <td style="width: 20%; text-align: right; vertical-align: top;">
                    <div class="meta-box">
                        <div class="meta-row"><div class="meta-label">Quotation #</div><div class="meta-value">{{ $sale->invoice_number }}</div></div>
                        <div class="meta-row"><div class="meta-label">Date</div><div class="meta-value">{{ $sale->sale_date->format('M d, Y') }}</div></div>
                        <div class="meta-row"><div class="meta-label">Expiry Date</div><div class="meta-value">{{ optional($sale->validity_date)->format('M d, Y') ?? 'N/A' }}</div></div>
                        <div class="meta-row"><div class="meta-label">Prepared By</div><div class="meta-value">{{ auth()->user()->name ?? 'System' }}</div></div>
                    </div>
                </td>
            </tr>
        </table>
        ...
        <table class="info-table">
            <tr>
                <td class="info-cell">
                    ...
                    <div><span class="info-label">Client:</span> <span class="info-value">{{ $sale->customer->name ?? 'N/A' }}</span></div>
                ...
                <td class="info-cell">
                    ...
                    <div><span class="info-label">Project:</span> <span class="info-value">{{ $sale->description ?: 'N/A' }}</span></div>
                    <div><span class="info-label">Delivery Timeline:</span> <span class="info-value">{{ optional($sale->delivery_date)->format('M d, Y') ?? 'N/A' }}</span></div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="section">
        ...
        <table class="items-table">
            ...
            <tbody>
                <tr class="alt-row">
                    <td>{{ $sale->description ?: 'Quotation summary' }}</td>
                    <td class="right">1</td>
    ...
    <div class="section">
        ...
        <div class="summary-box">
            ...
            <div class="summary-row"><span class="summary-label">Discount:</span> {{ $sale->currency }} {{ number_format($discount, 2) }}</div>
            <div class="summary-row"><span class="summary-label">Tax:</span> {{ $sale->currency }} {{ number_format($tax, 2) }}</div>
            <div class="summary-row"><span class="summary-label">Grand Total:</span> <span class="summary-total">{{ $sale->currency }} {{ number_format($total, 2) }}</span></div>
        </div>
    </div>

    <div class="section notes">
        <div class="section-title">Terms & Conditions</div>
        <div>{{ $sale->terms_conditions ?: 'Quotation valid until the expiry date. Payment and delivery terms apply.' }}</div>
    </div>

    <div class="section">
        <div class="section-title">Approval</div>
        <div class="approval-box">
            <div class="approval-line">Client Signature</div>
            <div class="approval-meta">Name: ________________________________</div>
            <div class="approval-meta">Date: _________________________________</div>
        </div>
    </div>

    <div class="footer">
        <p>Generated on {{ now()->format('F d, Y H:i:s') }} | Quotation #{{ $sale->invoice_number }}</p>
    </div>
</body>
</html>
